...
Secrets on the Worker: `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, `KEY_SECRET`, `LICENSE_SIGNING_KEY`; `STRIPE_WEBHOOK_SECRET` must be added after creating the Stripe endpoint `https://cleardisk.app/api/stripe/webhook` (events `checkout.session.completed`, `charge.refunded`, `charge.dispute.created`). Email uses the `EMAIL` send_email binding from `hello@cleardisk.app`; until the domain is onboarded to Cloudflare Email Sending the key is still shown on the thanks page and sends are logged as skipped. Free an activation slot with `npx wrangler kv key get --namespace-id c73aeb1e1928447780447d9ce14652dd key:<KEY>`, edit the `activations` array, and `kv key put` it back; revoke by setting `status` to `revoked`.

App 1.0.0 (build 6): `Sources/Core/License.swift` (key normalization, receipt verification), `Sources/ClearDiskApp/License.swift` (store, client, machine id, license file at `~/Library/Application Support/ClearDisk/license.json`), `UnlockSheet.swift`, gate inside `RemovalConfirmationSheet`, License… menu, `cleardisk://activate?key=` URL scheme. DMG sha256 `242e8a38649c2e2f6f63a3e2ebfd89b93bb2b0c984a8c831c9cb89b14829a174`, 3,061,506 bytes, GitHub release v1.0.0. Follow-ups: remove the unused `/api/checkout-status` route; onboard `cleardisk.app` to Email Sending; consider embedded Checkout only after measuring created-vs-completed sessions.
